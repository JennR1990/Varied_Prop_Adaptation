SizeofChangeModel <- function(par, schedule) {
  rotation<- schedule
  ...
  for (t in c(1:length(schedule))) {
    ...
    if (t > 1){
      if (rotation[t] - rotation[t-1] == 0){
        AP <- AP
      } else if (abs(rotation[t] - rotation[t-1]) == 15) {
        AP <- AP + .25
      } else if (abs(rotation[t] - rotation[t-1]) == 30) {
        AP <- AP + .5
      } else if (abs(rotation[t] - rotation[t-1]) == 45) {
        AP <- AP + .75 
      } else if (abs(rotation[t] - rotation[t-1]) == 60) {
        AP <- AP + 1
      }
    } else {
      AP <- AP
...
SizeofChangeMSE <- function(par, schedule, reaches) {
  
  bigError <- mean(schedule^2, na.rm=TRUE) * 10
  
  return( mean((SizeofChangeModel(par, schedule)$process - reaches)^2, na.rm=TRUE) )
  
}

SizeofChangeFit <- function(schedule, reaches, gridpoints=6, gridfits=6) {
  ...
  MSE <- apply(searchgrid, FUN=SizeofChangeMSE, MARGIN=c(1), schedule=schedule, reaches=reaches)
