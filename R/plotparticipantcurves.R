...
plotaverageperp<- function (){
  ...
  for (q in 2:33){
    ...
    starts<-c(seq(from= 2, to = 49, by = 4),seq(from= 50, to = 480, by = 4))
    stops<- starts+3
    stops[120]<- stops[120]-1
    
    locmeans<-c()
    for (u in 1:length(stops)){
      start<- starts[u]
      stop<- stops[u]
      locmeans<- c(locmeans, mean(variation_localization[start:stop,q], na.rm = TRUE))
    }
    title<- sprintf("p%d", q-1)
    plot(locmeans, col = "red", axes = F,cex.lab = 1.5,
         cex.main = 1.5,    xlab = "cycle (4 trials)",
         ylab = "Hand Location [°]", ylim = c(-30, 30), xlim = c(1,120), type = "l", main = title)
    
    reachmeans<-c()
    for (u in 1:length(stops)){
      start<- starts[u]
      stop<- stops[u]
      reachmeans<- c(reachmeans, mean(variation_reaches[start:stop,q], na.rm = TRUE))
    }
    
    lines(reachmeans*-1, type = 'l', col = 'Blue')
    z<-floor(z/4)
    lines(x = z[1:25], y = sizes[1:25], type = 'l', lwd = 2)
    lines(x = z[25:26], y = c(0,0), lty = 2, lwd = 2)
    lines(x = z[26:33], y = sizes[26:33], type = 'l', lwd = 2)
    ...
    axis(2, at = c(-30, -15, 0, 15, 30), cex.axis = 1.5,
         las = 2)
    s<-c(1,seq(from = 12, to = 120, by=3))
    axis(1, at = s, cex.axis = .75, las = 2)
  }
  dev.off()
